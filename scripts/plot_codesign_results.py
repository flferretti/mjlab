...
def create_design_comparison_plot(
  ...
  for design_idx, (ax, design_dict, (reward, cost)) in enumerate(zip(axes, X, F)):
    ...
    # Create bar plot with uniform color (all are continuous optimized values)
    bars = ax.bar(groups, tau_values, color="#4ecdc4", edgecolor="black", linewidth=1.5)
    ...
    ax.set_ylim(0, 160)  # Increased to accommodate expanded search space
    ax.set_title(
  ...
  plt.suptitle(
    "Design Parameters Across Pareto Set", fontsize=14, fontweight="bold", y=1.02
  )
  plt.tight_layout()
  plt.savefig(output_path, dpi=300, bbox_inches="tight")
...
def create_summary_plot(
  ...
  ax_heat.set_title(
    "Joint Torque Limits (Nm) per Design", fontsize=12, fontweight="bold"
  )
  ...
  # Overall title
  plt.suptitle(
    f"Co-Design GA Results: {len(X)} Designs Evaluated",
    fontsize=15,
    fontweight="bold",
    y=0.995,
  )
...
def print_summary(F: np.ndarray, X: np.ndarray, groups: list[str]):
  ...
  for group in groups:
    ...
    print(f"   • {group:14s}: τ_max = {tau:6.1f} Nm")
